...
int main() {
    ...
    // now only one completion handler run at the same time
    timer1.async_wait(strand.wrap([](auto...n){ timer_expired("timer1");}));
    timer2.async_wait(strand.wrap([](auto...n){timer_expired("timer2");}));
